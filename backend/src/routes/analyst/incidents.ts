...
import { sql, query, queryOne } from '../../lib/database.js';
import { putObject, getObjectBuffer } from '../../lib/storage.js';
import { requireAuth, requireRole, requireOrganizationAccess } from '../shared/middleware.js';
...
// Wszystkie endpointy wymagają autoryzacji analityka
...
async function downloadFile(req: Request, res: Response) {
	try {
		...
		const incident = await queryOne<{ organizationId: string; analystId: string | null }>(`
		...
		// lub które są w jego organizacji (dla plików które zostały już przeanalizowane)
		const hasAccess = incident.analystId === userId ||
						 (type === 'reports' || type === 'statements');
		...
		if (Array.isArray(fileData.fileData)) {
			fileMetadata = fileData.fileData.find((f: any) => f.filename === filename);
		} else if (typeof fileData.fileData === 'object' && fileData.fileData?.filename === filename) {
			// Dla pojedynczych plików (reports/statements mogą być pojedynczymi obiektami)
		...
		const fileBuffer = await getObjectBuffer(fileMetadata.path);
...
): Promise<{ path: string; metadata: { bucket: string; filename: string; mimeType: string; size: number } }> {
	try {
		...
		return {
			path: storagePath,
			metadata: {
				bucket: process.env.S3_BUCKET || 'bastiondesk-bucket',
		...
		try {
			...
			return res.status(500).json({
				...
				error: {
					...
					message: error instanceof Error ? error.message : 'Nie udało się przesłać raportu',
				},
			});
		}
...
async function uploadStatement(req: Request, res: Response) {
	try {
		...
		try {
			...
			return res.status(500).json({
				...
				error: {
					...
					message: error instanceof Error ? error.message : 'Nie udało się przesłać sprawozdania',
				},
			});
		}
		...
		await sql`
			...
			WHERE id = $3 AND "organizationId" = $4
		`, [uploadedStatement.path, JSON.stringify(uploadedStatement.metadata), id, organizationId];
