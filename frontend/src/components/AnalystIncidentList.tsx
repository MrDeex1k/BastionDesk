...
export function AnalystIncidentList({ type }: AnalystIncidentListProps) {
  ...
  return (
    <Card className="flex h-[600px] w-full max-w-4xl flex-col border-zinc-800 bg-zinc-900/50 shadow-xl animate-in fade-in duration-300">
      <CardHeader>
            ...
            <div className="space-y-4 pb-4">
              {data?.data.map((incident) => (
                <IncidentSummaryItem
                  ...
                  userAttachmentPath={incident.userAttachmentPath}
                  userDescription={incident.userDescription}
                  userId={incident.userId}
                  userName={incident.userName}
                  userScreenshotPath={incident.userScreenshotPath}
                />
              ))}
